...
# PD vs control_elderly (samma som tidigare)
...
def base_task(code: str) -> str:
    ...
    # hitta första gången vi har minst en siffra och sedan ev fler siffror
    # men här räcker: ta tills vi når en bokstav EFTER siffror (då är det variant)
    seen_digit = False
    base = ""
    for ch in out:
        if ch.isdigit():
            ...
            if seen_digit:
                # bokstav efter siffra => variant börjar, stoppa
                break
            base += ch
    return base
