...
def normalize_summary(df: pd.DataFrame, variant_name: str) -> pd.DataFrame:
    ...
    # Fall B: per-split-format (många rader) -> aggregera
    # Vi räknar bara om kolumnen finns.
    ...
    # (vi har redan rätt namn för coverage/decided/unsure/acc_decided, men auc_* är också ok)
    # Däremot har vi just nu "auc_mean" osv redan.

    # säkerställ att de finns (om någon saknas)
    for c in [
        "auc_mean", "auc_std",
        "coverage_mean", "coverage_std",
        "decided_rate_mean", "decided_rate_std",
